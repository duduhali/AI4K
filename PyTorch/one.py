...
# 训练数据集的加载器，自动将数据分割成batch，shuffle 代表打乱数据
...
class ModelG(nn.Module):
    def __init__(self):
        ...
        # 增加第三层反卷积层，输入num_features维，输出num_features/2维，窗口4，padding=0
...
# 在默认情况下，模型卷积核的权重分布是均值大概为 0，方差在 10^{-2}。 BatchNorm 层的权重均值是大约 0.5，方差在 0.2 左右。
# 使用如下初始化方式可以让方差更小，使得收敛更快。
def weight_init(m):
...
criterion = nn.MSELoss() #目标函数采用最小均方误差
...
torch.save(net,'ModelG.mdl')

'''验证最小均方误差模型的生成效果'''
# net = torch.load('ModelG.mdl')
if use_cuda:
    net = net.cuda()
# 绘制一批图像样本
