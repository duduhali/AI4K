...
class Predeblur_ResNet_Pyramid(nn.Module):
    def __init__(self, nf=128):
        ...
        self.RB_L1_1 = ResidualBlock_noBN(nf)
        self.RB_L1_2 = ResidualBlock_noBN(nf)
        self.RB_L1_3 = ResidualBlock_noBN(nf)
        self.RB_L1_4 = ResidualBlock_noBN(nf)
        self.RB_L1_5 = ResidualBlock_noBN(nf)

        self.RB_L2_1 = ResidualBlock_noBN(nf)
        self.RB_L2_2 = ResidualBlock_noBN(nf)

        self.RB_L3_1 = ResidualBlock_noBN(nf)
...
class EDVR(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        if self.is_predeblur:
            self.pre_deblur = Predeblur_ResNet_Pyramid(nf=nf)
            self.conv_1x1 = nn.Conv2d(nf, nf, 1, 1, bias=True)
        else:
            self.conv_first = nn.Conv2d(3, nf, 3, 1, 1, bias=True)

        layers = []
        for _ in range(front_RBs):
            layers.append( ResidualBlock_noBN(nf) )
