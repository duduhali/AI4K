...
class Block3D(nn.Module):
    def __init__(self,kernel_size, channel, reduction=16):
        super(Block3D, self).__init__()
        self.conv_du = nn.Sequential(
            nn.Conv3d(channel, channel // reduction, kernel_size, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv3d(channel // reduction, channel, kernel_size, padding=0, bias=True),
            nn.ReLU(inplace=True),
        )
...
class Block(nn.Module):
    def __init__(self,channel, reduction=16):
        ...
        self.conv_du = nn.Sequential(
            nn.Convd(channel, channel // reduction, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
            nn.Convd(channel // reduction, channel, 1, padding=0, bias=True),
            nn.ReLU(inplace=True),
        )
